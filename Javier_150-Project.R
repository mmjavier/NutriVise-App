...
Simplex = function(tableau, isMax){
  ...
  while(TRUE){
    ...
    if(tableau[rowSize,pivotCol] >= 0){ #Checks if it's a positive number
      ...
      if(pivotElement == 0){ #Checks if PE is equal to 0
        stop("Division by Zero") #Stops execution and prints the prompt
  ...
  if(isMax){ #Maximization Problem
    basicSolution = matrix(ncol = colSize-1, byrow = FALSE)
    for(k in 1:(colSize-1)){
      index = which.max(tableau[,k]) #Gets the index of the maximum element in kth Column of the tableau
      if(sum(tableau[,k])== 1){ #If sum = 1
        basicSolution[k] = tableau[index, colSize] #Store the value of the corresponding index from the solution column
      }else{ #When sum is not equal to 1
        basicSolution[k] = 0 #Store 0 in kth index of the basic solution
      }
    }
...
Foods = c("Frozen Broccoli","Carrots,Raw","Celery,Raw","Frozen Corn","Lettuce,Iceberg,Raw","Peppers,Sweet,Raw","Potatoes,Baked","Tofu","Roasted Chicken","Spaghetti W/ Sauce","Tomato,Red,Ripe,Raw","Apple,Raw,W/Skin","Banana","Grapes","Kiwifruit,Raw,Fresh",
          ...
          "Cheerios","Corn Flks,Kellogg'S","Raisin Brn,Kellogg's","Rice Krispies","Special K","Oatmeal","Malt-O-Meal,Choc","Pizza W/Pepperoni","Taco","Hamburger W/Toppings","Hotdog Plain","Couscous","White Rice",
          "Macaroni,Ckd","Peanut Butter","Pork","Sardines in Oil","White Tuna in Water","Popcorn,Air-Popped","Potato Chips,Bbqflvr","Pretzels","Tortilla Chip","Chicknoodl Soup","Split Pea & Hamsoup","Vegetbeef Soup","Neweng Clamchwd","Tomato Soup","New E Clamchwd,W/Mlk","Crm Mshrm Soup,W/Mlk","Beanbacn Soup,W/Watr")
